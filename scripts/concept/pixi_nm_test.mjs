// De-risk: normal-mapped sprite lighting in PixiJS v8 (Mesh + custom Shader).
// Renders the ship lit from left vs right to confirm Pixi can do what Three.js did.
...
const fragment = \`#version 300 es
  ...
  uniform vec3 uLight; uniform float uAmbient;
  void main(){
    vec4 a = texture(uAlbedo, vUV); if(a.a < 0.1) discard;
    vec3 raw = texture(uNormal, vUV).rgb * 2.0 - 1.0;
    vec3 n = normalize(vec3(raw.r, raw.b, raw.g));   // Blender iso camera-space decode
    float d = max(0.0, dot(n, normalize(uLight)));
    fragColor = vec4(a.rgb * (uAmbient + d * 1.2), a.a);
  }\`;
const shader = Shader.from({ gl: { vertex, fragment }, resources: {
  uAlbedo: alb.source, uNormal: nrm.source,
  lightUniforms: { uLight: { value: [-1,0.3,0.7], type:'vec3<f32>' }, uAmbient: { value: 0.28, type:'f32' } },
}});
...
window.__shots = { left: shot(-1,0.3,0.7), right: shot(1,0.3,0.7) };
window.__ready = true;
`;
...
for (const [k,d] of Object.entries(shots)) writeFileSync(join(root,'client/public/sprites/nm',`pixi_${k}.png`), Buffer.from(d.split(',')[1],'base64'));
console.log('wrote pixi_left/right.png');
