...
Screens & data: ships **dock** at HQ (HQ badge on the fleet tile; any order undocks
them). The **HQ City** and **Galaxy Map** screens are HTML overlays (top-right nav
buttons) — see `docs/HQ_AND_GALAXY.md`. The local cluster is generated by
`scripts/galaxy/gen_galaxy.mjs` (seeded, Delaunay/Bowyer-Watson) and **cached** to
`client/public/data/galaxy.json` — edit that file to tweak the map, or re-run the script.
**Every star is an enterable procedural system**: `client/src/sysgen.ts` expands a galaxy
entry into a deterministic `SystemSpec` (star class tint/size, planets, moons, belts,
glowing mineral fields); `loadSystem(id)` in `main.ts` tears down and rebuilds the system
view (home system 0 stays hand-authored). On the galaxy map, clicking the star the
selected ship is at ENTERS that system.
